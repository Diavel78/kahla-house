...
The edge estimate (`edge_pp`, which now finally populates the long-nullable `bot_picks.edge_pp` column) = `sharp_score × 0.40pp` + `aligned_splits_pp × 0.10pp`, hard-capped at 6pp. **The power rating does NOT feed sizing** (`MODEL_FEEDS_SIZING = False`) — a crude season-stats model has no business second-guessing Pinnacle, so its edge contribution is inert until CLV proves it correlates with beating the close. It's a watch-only divergence flag only. `_kelly_units` converts that to a full-Kelly fraction at the entry odds, takes a quarter of it (`KELLY_FRACTION = 0.25` — survives variance), and buckets it. **The coefficients are a conservative provisional guess; the new `clv_pp` column measures the bot's REALIZED edge so Stage-4 self-tuning can replace them with calibrated numbers.** The sharp gate (`SHARP_SCORE_MIN = 4`) is unchanged — it still decides real-pick (≥3u) vs forced-lean (1u); Kelly only chooses 3u vs 5u among gated picks. Whale (10u) stays disabled.
...
4. **Power rating** (`dossier.power_rating`, `handicapper_web.py:_power_rating`). An OUR-number-vs-the-market check built from the team-comparison stats the dossier ALREADY fetches (zero extra calls). Projects a margin from offense-vs-defense season averages (`_POWER_MODELS` per sport: MLB/NBA/CBB/NFL/NCAAF/NHL), converts to a win prob via a per-sport logistic, derives model fair lines, and compares to PIN's devigged fair (`edge_{home,away}_pp`) + a total lean. Crude (no SOS/Elo, and MLB ignores the starting pitcher — it uses team-season ERA) and trying to out-think the sharpest book in the world from public season stats, which is backwards. **It does NOT feed sizing** — `MODEL_FEEDS_SIZING = False` makes `_model_edge_for_side` return 0 regardless, so Kelly sizes off sharp + splits only. The model is surfaced as a watch-only `EXPERIMENTAL · NOT IN PICK` reference card + a `_buildReasons` context bullet, nothing more. Whether to ever give it sizing weight is a CLV question: once enough settled picks exist, bucket CLV by model-agree vs model-disagree; only if agreement actually beats the close do we flip the flag on. All four are web-only (`handicapper_web.py`) — the CLI dossier doesn't have suggestion/team-compare logic, so this doesn't violate the dossier-mirror rule (the verbatim-shared sharp-score helpers are untouched).
