...
**"Already logged" green Pick button (admin/bot_access only).** When the current user has a logged pick (pending or settled-today) on a game, that game's `Pick` button on the list turns **green** (`var(--green)`) and reads `✓ Pick` instead of the default blue `Pick` — so you can scan the slate and see at a glance which games you've already bet without re-opening each dossier. Driven by `_loggedMarketIds` (a Set of `market_id`s built in `loadData()` from the `/api/handicapper` pending+settled rows — which is why `market_id` was added to that endpoint's column list in `app.py`). `loadGames()` checks the set at render time; `_colorPickButtons()` re-colors already-rendered buttons after `loadData` refreshes (handles the games-render-before-picks-load gap, plus immediate re-color on log/delete/settle since all three call `loadData`). See the "Unit-tiered Pick-button color scheme" note below — green-logged is just one state of that machine. **Viewer-safe by construction:** `loadData` is gated to `_canBet`, so a view-only viewer never populates the set (and the render check is also `_canBet &&`) — their Pick buttons always stay blue, and they never learn what's been logged (consistent with the `@bot_required` isolation, gotcha #32).

**Unit-tiered Pick-button color scheme + new/upgrade signals (admin/bot_access only).** The games-list Pick button is a state machine painted by `_colorPickButtons()` from two sources: `_verdictCache` (the bot's current actionable picks per game) and `_loggedPicks` (what the user has already logged). States, in priority order:
- **`upgrade`** (**violet** `#a855f7`, pulsing — `.pick-btn.upgrade`; deliberately NOT gold so it's distinct from the 5u tier): you've logged a pick here BUT the bot now wants more — either your existing bet got **stronger** (current suggested units > the units you logged for that market_type → label `↑ 3u`) or there's an actionable pick in a **market you haven't bet** (label `✓ +Nu`). This is the headline fix for "my Nationals bet went 1u→3u and I'd never have known without re-opening it." Keyed on `market_type` (not side), so the opposite side of a market you already bet is NOT mistaken for new.
- **`logged`** (green `✓ Pick`): you've bet this game and nothing stronger/new is pending.
- **unit tier** (NOT logged, has an actionable pick): colored by the bot's recommended stake — `.u05` muted blue (0.5u, NRFI), `.u1` blue (1u), `.u3` amber (3u), `.u5` gold ring (5u). Louder = bigger stake. Label `Pick Nu`.
- **`lean`** (grey): evaluated in-window, no real pick ("skip").
- default blue `Pick`: outside the 2h window, not yet evaluated.

Driven by `_prefetchVerdicts()` — throttled (2-concurrent) background fetches of the SAME `/api/handicapper/dossier` a click uses, for in-window games (within 2h of start, INCLUDING already-logged ones so upgrades are caught), re-run on render + every 5 min. Caches `{picks:[{mt,units}], fetchedAt}` per `market_id` in `_verdictCache` (TTL 5 min); `picks` = every gate-cleared suggestion + the NRFI bet side (0.5u). `_loggedPicks` (market_id → Map(market_type→max units)) is built in `loadData` from **PENDING** rows only (you can't add to a settled bet). A `#pickLegend` strip above the list explains the colors (hidden for viewers via `_applyViewerMode`). **Why full dossier, not a sharp-score shortcut:** the verdict is the full model output (power model + Kelly + splits), identical to opening the dossier. **Cost:** zero Odds API credits (cache-only), negligible Vercel invocations. **Viewer-safe:** `_prefetchVerdicts`/`_colorPickButtons` early-return on `!_canBet`, cache + logged-sets stay empty, buttons stay plain blue.

**Market groups — never prompt the opposite side of a logged bet.** ML and SPR are the SAME directional bet (the bot picks one or the other, never both), so `_mktGroup()` files them under a shared `'side'` group; TOT and NRFI are their own groups. `_pickState(mid, mt, side, units)` classifies a candidate against `_loggedPicks` into `fresh`/`upgrade`/`logged`/`conflict`/`new`: a candidate whose GROUP is already logged (e.g. you bet Twins ML, bot now likes Pirates -1.5 SPR) is **`conflict`, NOT `new`** — so neither the dashboard button nor the dossier ever nudges you to add the opposite/correlated side of a game you're already on. Both `_colorPickButtons` (dashboard) and the suggestion-card renderer key off `_pickState`, so they agree. `_loggedPicks` stores `{units, side}` per market_type and the verdict-cache picks carry `side` for this.
