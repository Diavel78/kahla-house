...
| `/api/handicapper/prime-alert` | **cron-only** | shared-secret `?key=PRIME_CRON_SECRET` OR `FILLS_CRON_SECRET` (NOT Firebase) — pinged every ~5 min by `scanner-poll.yml`'s `fill_ping` job (minute-modulo gate in the workflow — see gotcha #38). Batched Telegram (Filled Bot) when games enter the **180min (3h)** prime window; deduped via `prime_alerts`. See the prime-window note in the Pick Bot section. |
| `/api/pm-snapshot` | **cron-only** | shared-secret `?key=PM_SNAPSHOT_SECRET` OR `FILLS_CRON_SECRET` — pinged every ~2 min by `fill_ping` (minute-modulo gate — gotcha #38). Logs free PMM+Kalshi cents per side for upcoming MLB games → `pm_snapshots` (the cross-confirm detector's memory + validation data). See "Prediction-market cross-confirm" section. |
| `/api/pm-snapshot-wc` | **cron-only** | shared-secret `?key=PM_SNAPSHOT_SECRET` OR `FILLS_CRON_SECRET` — pinged every ~2 min by `fill_ping` on **ODD** minutes (offset from `/api/pm-snapshot`'s even-minute tick so they don't stack Active CPU). The **World Cup 3-way logger** (foundation of the soccer pick engine, June 2026): logs PMM 1-X-2 (`side` home/draw/away, `market_type='ml'`, `line` NULL) cents for upcoming WC fixtures → `pm_snapshots`, **AND Kalshi 1-X-2 cents (`source='kalshi'`) as the cross-confirm second venue.** Built on `_build_worldcup` (proven ESPN+PMM reader) + `_pm_insert_changed` (dedup-on-change). **Markets spine:** `_wc_ensure_market` is the SOLE creator of WORLDCUP `markets` rows (ESPN ingest excludes WORLDCUP) — event_name is canonical `'{away} @ {home}'` with away/home sorted by country key → one fixture = one stable `market_id`, no dup rows. **Kalshi DOES carry the World Cup** (series `KXWCGAME`, 3-way per event: two country YES contracts + a `-TIE` draw contract; title `'{A} vs {B} Winner?'`). `_kalshi_wc_index` groups KXWCGAME markets by event into `{country_key|'draw': cents}` (draw = ticker `-TIE` suffix; teams keyed by `_wc_country_key` from `yes_sub_title`); `_match_kalshi_wc` matches each fixture by country pair → logs `source='kalshi'` home/draw/away rows alongside PMM. So soccer is a real cross-confirm sport now, same as MLB/NBA/NHL. `pm_snapshots.side` is free-text so `'draw'` needs no migration (`bot_picks.side` DID — migration 011). WORLDCUP markets rows feed the 3-way dossier/grading but are inert in the rest of the pipeline (the WC tab still lists via `_build_worldcup`; the standard games list + sport-counts don't read them). **FULL AUTO WC PICK ENGINE — COMPLETE (June 2026, steps 1-8):** (1) migration 011 `draw` side; (2-3) this logger + markets spine + Kalshi confirm feed; (4) `handicapper_web._devig_three_way`/`_wc_exch_fair` (3-way devig fair, verified sums to 1.0) + `_wc_sharp` (3-way PMM cent-movement sharp **with Kalshi confirmation gate** — mirrors `_xsharp_ml`: Kalshi disagrees on the picked side → veto, agrees → confirmed real pick, silent → demote to lean); (5) `_wc_research` (ESPN fifa.world form/record/rank, late-imports app, silent-fail); (6) `build_worldcup_dossier` + `GET /api/handicapper/worldcup-dossier` (@bot_required) — fair+sharp+research+suggestion (follow steam past the sharp gate, else 1u favorite lean, Kelly-sized) + PMM maker entry; pick POST accepts `side='draw'`; `/api/handicapper/worldcup` attaches `market_id` per match; (7) resolver: `WORLDCUP→soccer/fifa.world`, `_wc_match_espn` (both-orientation country matcher + alias), `_grade` soccer branch (**draw is DECISIVE — 'draw' wins on a level score, home/away ML LOSES on a draw, NOT a push**), CLV skipped (3-way); (8) frontend WC dossier modal (`openWcDossier`/`logWcPick`/`_copyWcForClaude`) with Win/Draw/Loss log buttons. **Remaining/TODO:** sharp score is thin until the cent-history clock accrues (~days); ESPN `_wc_research` shape (form/curatedRank) unverified from sandbox — confirm on a live tab-load; no soccer power model (the OUR-number is the exchange fair; deep read is Copy-for-Claude); spreads/totals not offered (1-X-2 ML only). |
| `/api/handicapper/paperlog` | **cron-only** | shared-secret `?key=PAPERLOG_SECRET` OR `FILLS_CRON_SECRET` — pinged every ~5 min by `fill_ping` (minute-modulo gate — gotcha #38). Auto-logs every gate-cleared bot suggestion (5h→1min window) → `pickbot_paperlog`, deduped on bet-change. Budgeted ~8s. See "Pick Bot paperlog" section. |
| `/api/vsin-snapshot` | **cron-only** | shared-secret `?key=VSIN_SNAPSHOT_SECRET` OR `FILLS_CRON_SECRET` — pinged every ~15 min by `fill_ping` (minute-modulo gate, aligned :00/:15/:30/:45; VSiN's scrape is 15-min cached so finer is wasted). Logs **Circa + DraftKings** handle%/bets% per side per market (ML/SPR/TOT) for upcoming games → `vsin_snapshots`, deduped on (handle,bets) change (`_vsin_insert_changed`, mirrors `_pm_insert_changed`). The sharp-money MOVEMENT CURVE — answers "when does sharp money hit Circa?" (Circa = sharp, no bet limits; DK = public, limited book). Reuses `_vsin_for_game` (the dossier's matcher) so a game's VSiN event is matched by team name. Feeds (a) model tuning and (b) the resolver's per-pick CLOSING stamp (`closing_vsin`, migration 012 — bot_picks AND pickbot_paperlog): at grade time `_closing_vsin` pulls the last pre-event_start `vsin_snapshots` rows (both sides, both books) and stamps the pick, exactly like CLV pulls the closing line. Paired with the BET-TIME read (`signal_blob.vsin`, already on every pick) it shows late-vs-early Circa money per pick. NRFI excluded (no first-inning splits). DDL: `kahla-scanner/supabase/vsin_snapshots.sql` (applied). |
...
0. **(FOUNDATIONAL — blocks the games list + board once `book_snapshots` stops) Schedule/event spine → ESPN.** Today the `markets` table — the schedule EVERYTHING joins on (`/api/handicapper/games`, pm-snapshot's per-game loop, the dossier, `market_id` on every `bot_picks` row) — is written ONLY by `kahla-scanner/scrapers/odds_api.py`. When odds_api dies nothing populates it, so the games list + pm-snapshot starve. **Decision (June 2026, user): ESPN is the event/schedule/grading spine wherever ESPN covers the sport; the exchanges supply ODDS matched onto ESPN events, NOT the other way around.** ESPN covers everything we run via `site.api.espn.com/apis/site/v2/sports/{grp}/{league}/scoreboard`: the 6 core sports already in `app.py:_ESPN_PATH`, PLUS **soccer per-league (`soccer/fifa.world` = World Cup, `usa.1`, `eng.1`, …)** and **MMA (`mma/ufc`, already used by `bot_picks_resolver._ESPN_PATH` to grade UFC ML).** Build a small ESPN→`markets` ingest (one active row per upcoming ESPN game; KEEP the existing UUID `market_id` so `bot_picks`/dossier joins are untouched) to replace odds_api's market-creation role; pm-snapshot + dossier then attach PMM+Kalshi quotes by the team-name + start-time match they already do. **Venue-neutral upside:** a game shows even before an exchange lists its market (vs. PMM-as-spine, where the slate is hostage to the venue's listing time). **The ONE exception (user): MMA method-of-victory props** — `mma/ufc` grades the winner/ML only, so SPR/TOT method bets stay manual-settle, exactly as today. Verify `soccer/fifa.world` shape FROM VERCEL (sandbox WebFetch is WAF-blocked; the Flask runtime reaches ESPN fine — the resolver already does). **DONE (June 2026): the World Cup tab is already on the ESPN spine** — `_build_worldcup` in `app.py` uses `soccer/fifa.world` (via `_espn_scoreboard_raw`) as the schedule + live-score source and matches PMM 1-X-2 odds on by canonical country name (`_WC_COUNTRY_ALIASES` / `_wc_country_key`; verified 36/36 ESPN matches joined to PMM, zero unmatched). Country variants handled: Korea Republic/South Korea, Turkiye/Türkiye, IR Iran/Iran, Cote d'Ivoire/Ivory Coast, Cabo Verde/Cape Verde, Czechia/Czech Republic, Congo DR/DR Congo, Bosnia-Herzegovina. **The general ingest is LIVE (cut over June 2026):** `kahla-scanner/scripts/ingest_espn_markets.py` runs as a step in `scanner-poll.yml` (`--exclude WORLDCUP --commit`) BEFORE odds_api each tick, and **odds_api is find-only** — `_find_or_create_market` returns None instead of creating when `ESPN_MARKETS_SPINE=1` (set on the odds_api step). So **ESPN is the sole market creator**; odds_api only attaches snapshots to existing ESPN rows. This killed two things at once: duplicate markets, AND The Odds API's broad `mma_mixed_martial_arts` bucket minting GLORY-kickboxing rows under "UFC" (ESPN's `mma/ufc` is UFC-scoped). UUID `market_id` preserved (ESPN reuses existing rows via the same matcher). **WORLDCUP excluded** from the cron ingest — the World Cup tab sources ESPN+PMM directly and soccer isn't gradeable yet. **UFC SCHEDULE is ingested** (mma/ufc = flat list of bouts; iterate ALL `competitions[]` per event — reading `[0]` only undercounts to the main event; competitors under `athlete`, mirrored from `bot_picks_resolver._ufc_match_espn`); the one true exception is narrower than "MMA" — only **method-of-victory PROPS** can't auto-grade (winner/ML grades fine) and stay manual-settle. **One-time cleanup of pre-cutover garbage** done via `ingest_espn_markets --commit --prune` (UFC: 17 real fights kept, 67 kickboxing/dupe/nonsense rows deactivated). `--prune` (deactivate active FUTURE markets ESPN didn't return; guarded to sports where ESPN returned a slate, future rows only) is a **manual tool, NOT on the cron** (recurring prune of MLB/NBA risks wiping the board if ESPN under-returns a tick). The `espn-markets-ingest.yml` workflow stays for manual dry-runs / prune / backfills. **Remaining cutover bits still pending below** (CLV, fair anchor, board) are about ODDS sources, not the schedule — the schedule spine is done.
...
**Lean chips (June 2026, per user: "at the end of the day I still want picks, even 1u").** When a game has NO gate-cleared ML/SPR/TOT pick, the games-list line 2 renders the TOP forced lean as a tappable amber-dashed `.pick-chip.lean` chip (`lean · ML Reds +115 · 1u`) instead of the dead "no pick — lean" note — one-taps to log at 1u like any chip (logged/upgrade/conflict states apply; no make/take book call for leans). The note now only shows when there's no PIN data at all.
...
**Two-line games list + one-tap log chips (admin/bot_access only).** Every game row is **two lines, always** (no expand): line 1 = kickoff time + matchup + a neutral **`Details ›` button** (opens the full dossier via `pickGame`); line 2 = the bot's **recommended bets as one-tap log chips**, or a grey note. This is the "fast lane vs deep dive" split — tap a chip to blind-follow, or hit Details for the full read. Rendered by `_renderPickLines()` per row:
- **> 2h to start →** grey `lean only, outside pick window` note (no chips — the read isn't actionable that far out; matches the `VERDICT_WINDOW_MS` 2h gate, which is also when the prefetch kicks in).
- **≤ 2h, verdict cached →** one chip per recommended bet, e.g. `ML Reds +115 · 1u`. **Tapping the chip logs that bet instantly at the recommended units** via `oneTapLog()` (no modal; body built by `_suggestionToBody`, maker entry = PMM bid, reasons auto-filled — mirrors `submitLog`). Chip color = unit tier (`.u05` muted / `.u1` blue / `.u3` amber / `.u5` gold) OR state via `_pickState`: green `✓` `logged`, violet pulsing `↑` `upgrade` (still one-taps to add), grey `⚠` `conflict` (opposite/correlated side of a logged bet — tap opens Details, never one-taps). `oneTapLog` also re-checks `conflict` server-side-of-the-fold as a safety.
- **≤ 2h, not yet evaluated →** `evaluating…` until the prefetch lands.
- **no gate-cleared pick →** `no pick — lean`.

Driven by `_prefetchVerdicts()` — throttled (2-concurrent) background fetches of the SAME `/api/handicapper/dossier` Details uses, for in-window games (within 2h, INCLUDING logged ones so upgrades surface), re-run on render + every 5 min + every 60s tick (so a game crossing into the window flips from the note to chips). Caches `{picks:[{market_type, side, units, _chip, _body}], fetchedAt}` per `market_id` in `_verdictCache` (TTL 5 min); `picks` = every gate-cleared suggestion + the NRFI bet side (0.25u), each carrying a prebuilt chip label + ready-to-POST body. `_loggedMarketIds` (any logged pick on a game) + `_loggedPicks` (market_id → Map(market_type → {units, side}), PENDING rows only) drive the logged/upgrade/conflict states. Legend strip `#pickLegend` above the list (hidden for viewers via `_applyViewerMode`). **Cost:** zero Odds API credits (cache-only dossier), negligible Vercel invocations. **Viewer-safe:** `_renderPickLines`/`_prefetchVerdicts` early-return on `!_canBet` (line 2 stays empty), so viewers only ever see matchup + Details.
...
**Make-vs-take execution engine (June 2026) — the directive chip, CROSS-VENUE (Polymarket + Kalshi).** The maker-only default isn't always right: as tip nears, or when the book is heavily bid-stacked, a resting maker MISSES the bet. So each gated suggestion gets a **live order-book verdict** picking the single cheapest fillable option across **FIVE** choices (all-in cents = odds + fee/rebate, leaning maker): **MAKE Polymarket** (rest at bid; fillable only when ask-heavy = more sellers than buyers, so sellers come to your bid), **MAKE+ Polymarket** (rest at **bid+0.5¢** — Polymarket quotes half-cent ticks; stepping in FRONT of the whole bid queue for half a cent turns "rest behind a 132k wall and never fill" into "fill on the next sell," still a maker earning the rebate; covers the balanced/mild-bid zone a plain rest sits behind), **TAKE Polymarket** (cross at fee-inclusive ask), **TAKE Kalshi**, **MAKE Kalshi**. A blowout (bid_heavy, e.g. 4M bids / 3 sells) makes both Poly makers unfillable → falls to the cheaper TAKE. No US-sportsbook routing — both venues are deep exchanges the user bets; Kalshi matters because it's the equally-deep second venue + sometimes the cheaper TAKE (Poly maker is fee-advantaged via the rebate, Kalshi CHARGES maker fees, so Poly maker usually wins on cost). **Kalshi is cross-shopped on ALL main markets:** ML via `_kalshi_side_book` (`KXMLBGAME`), TOTAL/SPREAD via `_kalshi_line_book` (`_KALSHI_LINE_SERIES` — `KXMLBTOTAL`: YES="Over (N-0.5) runs", suffix N=line+0.5; `KXMLBSPREAD`: alt run-line ladder, YES="{team} wins by over (N-0.5) runs", suffix `{fav}{N}`, N=margin+0.5, std −1.5 runline = `{fav}2`; under/dog = `_invert_book`). MLB-only line series for now (add to `_KALSHI_LINE_SERIES` + confirm suffix encoding per sport via `/debug-kalshi-discover`-style probe). The chip shows the actionable POST price (`MAKE+ Polymarket 47.5¢`); the LOGGED entry uses `entry_cents` (effective all-in, fee/rebate baked in) so CLV/to-WIN stay honest (they differ for a maker by the rebate). **Backend `GET /api/handicapper/make-take`** (`@bot_required`; params `slug`,`units`,`starts_in_min`,`line`,`inverse`,`sport`/`away`/`home`/`side`/`market_type`) reads the Polymarket book via `_pmm_book` + the Kalshi book → `_cross_book_signal` (the single-venue `_book_signal` is now debug-only). Verify any market on `/debug-crossbook?sport=mlb&side=over&market_type=total`. The per-venue read still uses:
...
>
> **Frontend (`handicapper.html`) — the directive rides the GAME CARD, the read lives on Details.** Per the user's mental model: the **games-list line-2 chip is "do this, this way"** and **Details is the explanation** — so the make/take pill's PRIMARY home is the game card, NOT the dossier. `_prefetchVerdicts` (the same background pass that builds the line-2 pick chips for in-window games) calls `_fetchMakeTakeVerdict` per gate-cleared pick that has a `pmm_slug`, attaches `_mt`/`_mtChip` (`_mtShort` → `MAKE Polymarket 46¢` / `TAKE Polymarket 54¢`), and `_renderPickLines` paints a quiet `.mt-pill` (green `.make` / amber `.take`) riding next to the bet chip inside a `.pick-chip-row` (hidden once logged / on a conflict — the directive is moot then). It refreshes with the 5-min verdict cache (TTL), NOT the 60s kickoff tick. **`_fetchMakeTakeVerdict` also populates `_makeTakeCache`, so a one-tap log from the chip enters at the rec** (`_applyMakeTakeToBody` reads that cache). The **dossier** still renders its own `.mt-chip` + a verbose `.mt-detail` read (book touch / spread / fee / imbalance / the engine's one-line why) via `_loadAllMakeTake` at the end of `renderDossier` (re-runs on the 30s dossier loop) — that's where the full "why" lives. `_makeTakeCache` is keyed `mid:mt:side`. **History note: this was first built ONLY on the dossier (wrong surface — buried the directive behind Details); moved onto the game card per the original design.**
...
> - **Prime-window Telegram alert (`GET /api/handicapper/prime-alert`, June 2026)** — a batched heads-up (via the **Filled Bot** — reuses `_send_fill_telegram` + `FILLED_BOT_TOKEN`/`FILLED_BOT_CHAT_ID`) when a cluster of games crosses into the 90-120min prime window, so the user picks in the window that prints. **ONE message per sport** ("5 MLB games entering the prime betting window · first pitch 11:00–11:15 AZ"), NOT one per game. Dedup via the `prime_alerts` table (one row per pinged market; `kahla-scanner/supabase/prime_alerts.sql`, already applied). Batching: each tick finds active un-pinged games starting in `[now+90, now+150]` (the +150 look-ahead pulls a whole cluster into one alert), groups by sport, fires a sport only when its EARLIEST fresh game is ≤`now+120` (genuinely in prime, not pre-firing a far cluster), then marks the whole batch. Quiet before 7am AZ. **Auth: `?key=` matched to `PRIME_CRON_SECRET` OR (fallback) `FILLS_CRON_SECRET`** — so NO new secret needed. Pinged every ~5 min by a curl step in `scanner-poll.yml`'s `fill_ping` job (no new cron-job.org job; minute-modulo gate — gotcha #38). All creds already exist → zero new setup.
| Conf | Units | When |
|---|---|---|
...
| medium | 3u | Gate cleared AND ¼-Kelly stake ≥ `KELLY_MED_PCT` (2.0%) and < `KELLY_HIGH_PCT` (3.0%) — strong multi-signal agreement. Uncommon. |
| high | 5u | Gate cleared AND ¼-Kelly stake ≥ `KELLY_HIGH_PCT` (3.0% bankroll) — near-cap multi-signal read. Rare. |
...
> **Size-up is MONEYLINE-ONLY (June 2026 — `SIZE_UP_MARKETS` in `handicapper_web.py`).** A 365-pick live review of sized-up (3u/5u) picks by market: **ML 19-12 (61%) +27.4u, TOT 9-11 (45%) -7.2u, SPR 0-10 (0%) -12.0u** — spreads never won a single sized-up bet, in either era. The ¼-Kelly stake predicts wins for moneyline but is pure variance on spreads (a leveraged restatement of the same ML directional bet — same edge, more variance) and totals (sized off line movement alone; the total-side veto already flags those leaks). Same failure mode as the whale tier — a signal that's real for one market applied where it isn't. So **only ML earns 3u/5u; SPR + TOT cap at 1u** (the loop in `_suggest_picks` forces `units=1` for any `market_type not in SIZE_UP_MARKETS`). This REPLACED the earlier totals-capped-at-3u guard. Bankroll defense, not a model change — SPR/TOT are still bet, just at 1u. Re-check the per-market sized-up PnL (`select market_type, sum(pnl_units) from bot_picks where units>=3 group by 1`) before adding a market back to `SIZE_UP_MARKETS`.
...
**Bet determination = model fair vs the POLYMARKET line.** There's no NRFI line in our Odds-API feed, so `pmm_markets.lookup` pulls Polymarket's first-inning market (`_classify_first_inning` sniffs it by question text — it lives OUTSIDE the ml/spread/total buckets; returns it as `pmm_data["nrfi"]`). `_nrfi_model` then computes the **edge per side = model_fair_prob − PMM maker-entry (bid) prob**; the +EV side with edge ≥ `NRFI_EDGE_MIN_PP` (3pp) is THE bet — its button lights up green and `entry_price` = the PMM bid (maker-only entry, like the other markets). No edge → pass; **no PMM market → model fair shown for reference, no bet button** (can't price an edge without a line). The model-vs-baseline `lean` is kept as secondary context only. **PMM's NRFI schema is UNVERIFIED from the sandbox** — `lookup`'s `diag.all_markets` dumps every market PMM returns for the event (surfaced via Copy-for-Claude → `pmm_meta.diag`) so we can confirm/tune `_classify_first_inning` against a real game; if PMM turns out not to list NRFI at all, the card falls back to reference-only. **Sizing: flat 0.5u** (`confidence='low'`) — bumped from 0.25u June 2026 after NRFI graded 15-6 (71.4%) over its first 21 picks, the best hit rate of any market; 0.25u was leaving money on the table. Still flat (no Kelly) while the model calibrates. Migrations `009_half_unit.sql` (numeric units) + `010_quarter_unit.sql` (0.25 in the CHECK) cover both sizes; 0.5 was already valid. `entry_line` is NULL. The unit size is wired in `templates/handicapper.html` (`logNrfi` seed + the line-2 chip body), NOT in the model — change it there if you re-tune.
...
MMA + soccer are omitted from THIS map only because the Odds Board doesn't ingest them — NOT an ESPN limitation. ESPN does have `mma/ufc` (used by `bot_picks_resolver._ESPN_PATH` to auto-grade UFC ML) and `soccer/{league}` (e.g. `fifa.world` = World Cup, `usa.1` MLS, `eng.1` EPL). Add them here only if the board ever shows those sports.
