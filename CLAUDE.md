...
**The dossier speaks the same color language.** When you open a game, the suggestion card's primary button is tier-colored by stake (`_tierCls` → `.t3` amber / `.t5` gold / `.t05` muted; 1u stays blue), as is the market-grid "Log bot pick" button — so you can see which side is the big play. If you've already logged that market_type, the card shows a badge: **violet `↑ ADD — you have Nu`** when the bot now wants more (button label becomes "Add to Nu"), or green `✓ logged Nu` when it matches. NRFI/YRFI card buttons use `.nrfi-actions` styling (the +EV side is a green `.bet` button) to match the rest — they were previously unstyled/garish.
