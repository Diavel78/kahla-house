...
39. **Post-cutover SPR/TOT picks need the exchange fair + entry on the SAME line (the "Yankees +1.5 @ -285, fair -100" phantom).** PIN is dead (book_snapshots frozen June 19), so the SPR/TOT suggestion fair comes from `exch_current` (PMM devig at the at-the-money line — e.g. a runline's -1.5 @ 50¢ → -100). But `pmm_bid_american` was read from `blk['polymarket'][side]`, a PMM quote attached at whatever line `best_line_for` chose — and `best_line_for` was keyed off the **now-dead PIN line**, so post-cutover it can land on a DIFFERENT line (the favorite +1.5 @ -285). Result: entry (+1.5/-285) and fair (-1.5/-100) on different lines → an incoherent pick that ALSO **dodged the SPR chalk filter** (which guards on `fair_american <= -150`; a bogus -100 sails through). ML + TOT were usually fine (their PMM line matched the exch line). **Fix (live):** `_suggest_picks` has a **line-consistency guard** — for SPR/TOT, only trust the PMM quote when `pmm_line == exch_fair_line`; else drop the quote so the entry falls back to the exchange fair at ITS own line. **Don't remove it** without first fixing `_attach_pmm_to_odds`/`best_line_for` to choose the PMM line off the exchange at-the-money line (not PIN). Diagnose recurrences by checking whether a SPR/TOT pick's displayed line == its fair line. **Related (same cutover):** the dossier freshness label now reads `data_freshness.exch_latest_captured` (pm_snapshots, the live ~2-min logger) as "exch Nm ago" — NOT the retired `odds_ingest_runs`/frozen `book_snapshots` ("cron 19h ago" was a dead-feed artifact, not an outage).
