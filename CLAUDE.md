...
- `kahla-scanner/supabase/bot_picks_migrations/` — manual SQL migrations for `bot_picks` schema changes. Run when the DDL changes (constraint additions, column renames, etc.) via `kahla-scanner/scripts/run_sql.sh -f <file>` (or, if creds aren't wired, the Supabase SQL editor). Numbered `00N_description.sql`. Idempotent. **All current migrations are already applied to the live DB** (verified May 2026): `001_units_whale_tier.sql` (added 10u + whale confidence), `002_to_win_pnl.sql` (recompute pnl_units to to-WIN model), `003_resolver_heartbeat.sql` (resolver_runs heartbeat table — read by `/api/handicapper`), `004`–`006` (legacy Polymarket-link columns — Pick Bot no longer auto-links, see gotcha #26), `007_clv.sql` (added `clv_pp` — Closing Line Value per pick, computed by the resolver from PIN's closing line in `book_snapshots`).
