...
> - **Prime-window sizing concentration (`_timing_window`)** — picks made 90-120min pre-tip carried the entire edge (38 picks, 68.4%, +27u) while 60-90 was modest (+3.9u) and the last hour was mush (51 picks, 52.9%; totals negative). So **only `prime` (90-120min) picks can size past 1u via Kelly** — everything else with a known kickoff time is **capped at 1u** (caps DOWN only; unknown kickoff → no cap). Windows: `<60` late (amber badge), `60-90` normal, `90-120` prime (green badge), `>120` early/outside the 2h chip window. Picks carry `timing_window`/`size_capped`/`late_capped`/`conflict_reason` into `signal_blob` so the next review can bucket by them. Needs `sport`/`home`/`starts_in_min` threaded into `_suggest_picks` (done in `build_dossier`).
