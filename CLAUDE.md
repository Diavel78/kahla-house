...
**Make-vs-take execution engine (June 2026) — the directive chip, POLYMARKET-ONLY.** The maker-only default isn't always right: as tip nears, or when the book is heavily bid-stacked, a resting maker MISSES the bet. So each gated suggestion gets a **live order-book verdict** telling the user exactly how to enter: `MAKE Polymarket 46¢` (rest a limit at the bid, fee-free, the cheapest+patient play) or `TAKE Polymarket 54¢` (cross now at the fee-inclusive ask). **No sportsbook routing — the user is Polymarket-exclusive** (best-execution routing to DK/FD/etc. was built then REMOVED June 2026 by user request; it's in git history if ever wanted). **Backend `GET /api/handicapper/make-take`** (`@bot_required`; params `slug`,`units`,`starts_in_min`) reads the Polymarket book via `_pmm_book` → `_book_signal`:
