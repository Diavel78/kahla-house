...
| `/api/handicapper/prime-alert` | **cron-only** | shared-secret `?key=PRIME_CRON_SECRET` OR `FILLS_CRON_SECRET` (NOT Firebase) — pinged every ~1 min by `scanner-poll.yml`'s `fill_ping` job. Batched Telegram (Filled Bot) when games enter the 90-120min prime window; deduped via `prime_alerts`. See the prime-window note in the Pick Bot section. |
...
> - **Prime betting window = 60-120min (`_timing_window` + `_PRIME_WINDOW`)** — the bot's edge lives in the 1-2h pre-tip window: 60-120min = 63.9% / +31u over 61 picks. **Only `prime` (60-120min) picks can size past 1u via Kelly** — `late` (<60min, the mushy last hour, 52.9%) and `early` (>120min, outside the 2h chip window) are **capped at 1u** (caps DOWN only; unknown kickoff → no cap). The window was widened from 90-120 → **60-120 (June 2026)** because 60-90 is genuinely solid (56.5%, +3.9u) and a 30-min betting window was too tight to act on. **BUT the +27u HAMMER is concentrated in the 90-120 core** (68.4%, 38 picks) — so `_is_prime_core` (90-120) is tagged separately into `signal_blob.prime_core`, letting the next review compare the 60-90 half against the proven core before trusting sized-up 60-90 picks as much. Windows now: `<60` late (amber badge, capped 1u), `60-120` prime (green badge + games-list glow + size-up, `_isPrime` mirrors this in `handicapper.html`), `120-150` **early** (the picker/evaluator was extended out to 2.5h — `EVAL_WINDOW_MAX`/`VERDICT_WINDOW_MS` — because sharp money often moves the line BEFORE the 2h mark; these are actionable at **1u, capped, no glow**, and measured via `clv_pp` before any promotion to sizing), `>150` far (outside the evaluator → "lean only, outside pick window"). **This required widening the cron's 5-min cadence bucket to 2.5h** (gotcha #27 / the adaptive-cadence note) so the 120-150 dossiers aren't stale — extend one without the other and you're picking off 15-min-old lines. Picks carry `timing_window`/`prime_core`/`size_capped`/`late_capped`/`conflict_reason` into `signal_blob`. Needs `sport`/`home`/`starts_in_min` threaded into `_suggest_picks` (done in `build_dossier`). **NOTE:** the prime-window Telegram alert (`/api/handicapper/prime-alert`) still fires at the **120-min top-of-window ENTRY** (its 90/120/150 constants are about entry detection + batching, NOT the window width) — that's correct: you want the heads-up as games cross IN, giving the full 60min to act. Don't "sync" those to 60-120.
