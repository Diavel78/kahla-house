...
| `GET /api/sharp-bot` | **Admin** | Phase 4 paper-bet tracker payload. Returns `pending` (every `paper_bets` row with `status='pending'`, no age cap), `settled` (last 30 days), `stats` (per-bot rollup over the 30d-settled window: graded count, hit rate excluding pushes, total units, ROI per bet), plus four extra rollup blocks added so a phone can review the bots without Supabase: `stats_all_time` (every graded paper_bets row), `stats_7d` (picks whose `event_start` was in the last 7d — bucketed by event date, not settled date, so off-season weekends don't dilute), `stats_by_market` (per-(bot, market_type)), and `heartbeat` (per-bot total / graded / pending / last_pick_at / last_settle_at / overdue_pending — overdue = `status='pending'` AND `event_start < now`, the resolver-stuck signal). Single endpoint, no caching — paper_bets queries are tiny. Browser-friendly JSON view at `/sharp-bot.json` (admin-gated client-side) with a Copy button. |
...
Gates (`SHARP_SCORE_MIN = 4`) were left unchanged when recency weighting shipped — the new score is MORE selective than the old at the same threshold (a sharp_score of 4 now means "4 cents of weighted-toward-recency PIN movement" instead of "4 cents over the whole pre-game window"). Reconsider gates after observing the new score distribution for a few days.
