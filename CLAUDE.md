...
3. **(REMAINING — the only Pick-Bot-adjacent piece left) Odds Board rebuild** (`/odds`, `templates/odds.html` + `app.py:/api/odds*`): columns = Polymarket + Kalshi cents; movement bar / sparklines / chart modal / openers read `pm_snapshots` instead of `book_snapshots`; live-game freeze + ESPN scores keep working (ESPN is independent). `/api/openers/scanner` → earliest exchange snap per side. **Until this lands the board is FROZEN on its last book_snapshots values** (won't 500 — gotcha #33 covers empty — just stale). Also switch `app.py:_clv_pin_close_pair` (dashboard CLV) here.
