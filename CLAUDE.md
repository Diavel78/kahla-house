...
- **🚨 STEAM** — for each book on each (market_type, raw_side), computes the implied sharp side from THAT book's move via `_move_sharp_side()` (line direction first for SPR/TOT, vig fallback). A book only counts if its move clears the noise floor: `STEAM_MIN_MOVE_CENTS = 5` (price moves) or `STEAM_MIN_LINE_MOVE = 0.5` (line moves). Without those floors, 1-3 cent normal vig drift counted as a directional move and tripped false STEAM alerts on tight markets. Groups books by `(market_type, sharp_side)`; fires when ≥`STEAM_BOOK_COUNT` (5) books point at the same sharp side **AND PIN is one of them**. Single book counted once per market regardless of which raw side reported the move. The PIN-confirmation gate kills the failure mode where 4-5 retail books re-juiced together while PIN sat still or moved the opposite direction (`Toronto @ Twins` ML alert: PIN moved 1c — below floor — and pointed opposite the retail consensus; `D-backs @ Cubs` TOT OVER alert: PIN line flat, vig pointed UNDER while retail raised lines).
