...
| `BOOK_CLUB_FROM_EMAIL` | Vercel env (optional) | Invite "from" address. Defaults to `Kahla House Book Club <bookclub@thekahlahouse.com>`. The sender domain (`thekahlahouse.com`) must be **verified in Resend** or sends fail; for quick testing use Resend's `onboarding@resend.dev`. |
