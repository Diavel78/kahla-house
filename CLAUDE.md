...
**The Odds API subscription is CANCELED, paid through June 25.** After that date `book_snapshots` stops receiving rows and every PIN-dependent surface goes stale — the cutover to PMM+Kalshi must be COMPLETE first. This section is the working plan; **delete it (replace with a 2-line stub) when the cutover is done.** User decisions already made: rebuild the Odds Board on Poly+Kalshi (don't kill the page); exchange data is the sole odds source going forward; Kalshi mid is the independent fair anchor for pricing PMM entries (you can't measure PMM's mispricing against PMM itself).

**Done (June 12):**
- ✅ Phase 0 — `pm_snapshots` carries PMM spreads/totals per line (`market_type`+`line`, migration applied; SPR/TOT history accrues from June 12 ~01:00 UTC; ML since ~June 7). Kalshi still ML-only.
- ✅ Step 1 — **exchange-based sharp score in shadow mode** (`_pm_history`/`_xsharp_ml`/`_xsharp_two_sided`/`_attach_xsharp`, handicapper_web.py). Every market's `movement` carries `x_side`/`x_score`/`x_n`; every suggestion + paperlog row carries `x_score`/`x_side`/`x_agree`. PIN stays primary until verified.
...
1. **(~June 13-16) CLV → exchange close.** `bot_picks_resolver._compute_clv` + `app.py:_clv_pin_close_pair` grade vs PIN's closing devig; switch to the exchange closing mid from `pm_snapshots` (last pre-`event_start` cents for the side). Keep writing PIN CLV in parallel where available until the 25th (column per source or stamp the source in the row) so historical CLV stays comparable.
2. **(~June 13-16) Dossier fair anchor → exchanges.** `pin_current`/devig fair (the Polymarket limit-order target + edge base) moves to Kalshi mid as primary independent anchor, PMM mid fallback; the PMM push-rate projection machinery becomes unnecessary where the quote is already at PMM's own line. The dossier's per-book odds grid (retail reference prices) dies with the feed — show the two exchanges.
3. **(~June 16-20) Odds Board rebuild** (`/odds`, `templates/odds.html` + `app.py:/api/odds*`): columns = Polymarket + Kalshi cents; movement bar / sparklines / chart modal / openers read `pm_snapshots` instead of `book_snapshots`; live-game freeze + ESPN scores keep working (ESPN is independent). `/api/openers/scanner` → earliest exchange snap per side.
4. **(~June 18-22) Flip the sharp-score primary** once the shadow data verifies. **Flip criterion:** paperlog rows where both scores exist show x_agree ≥ ~80% on the side AND no systematic scale bias (compare score distributions; tune `_X_PP_SCALE` if exchange scores run hot/cold vs PIN). Review query: `select x_agree, count(*) from (select (signal_blob->>'x_agree')::boolean as x_agree from pickbot_paperlog where signal_blob->>'x_score' is not null) t group by 1;`
5. **(June 22-25) Retire the machinery:** remove the odds-ingest step from `scanner-poll.yml`; retire `scrapers/odds_api.py`'s adaptive cadence + `_should_fire` + the xconfirm→`odds_pull_requests` trigger (its only purpose was rationing paid PINN pulls — pm-snapshot keeps running as the primary ingest); CLI dossier (`kahla-scanner/scripts/handicapper.py`) either gets the same `_pm_history` port or is retired (decide then — the mirror rule applies only while both live); `handicapper_backtest.py` similarly. `ODDS_API_KEY` env/secrets can be deleted after the 25th. snapshot-cleanup keeps running until `book_snapshots` drains past retention, then can sweep that table too.
6. **(June 25) Verify nothing 500s with zero fresh `book_snapshots`** — grep for remaining `book_snapshots` readers before declaring done (`/api/odds*`, `/api/clv`, resolver, dossier, sparklines, make-take routing leftovers).
