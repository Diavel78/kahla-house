...
- `kahla-scanner/scripts/tune_prime_window.py` — **weekly prime-window auto-tuner** (the agentic self-adjust). Grid-searches settled gate-cleared ML/SPR/TOT `pickbot_paperlog` rows, optimizes mean per-pick **unit-ROI** (sizing-neutral → isolates the TIMING edge) with a **CLV guardrail** + stability rails (min sample 25, max 30-min/run bound move, min ROI gain to change). Writes the best `(prime_lo, prime_hi)` to the `pickbot_tuning` singleton row; `handicapper_web._load_prime_window` reads it at runtime (5-min cache, `_PRIME_WINDOW` constant fallback). Run via `.github/workflows/tune-prime-window.yml` (weekly Mon 12:00 UTC + `workflow_dispatch` dry-run/lookback). `kahla-scanner/supabase/pickbot_tuning.sql` (applied to live DB June 2026). Safe by construction — leaves the window alone on thin data. **Sandbox note:** can't run the script here (supabase py client hits the cffi/crypto load bug) — validate the metric via `run_sql.sh` SQL instead.
...
- ✅ BONUS — **prime window is now DATA-DRIVEN** (weekly auto-tuner, see "Pick Bot — handicapper" + Key Files). The hand-set (60,180) was the weak link: live data shows the "proven 90-120 hammer" is now the WORST bucket (-0.32/unit) while 30-60 (+0.32) and 120-180 (+0.12) win — the edge is BIMODAL with a hole at 90-120. ⚠️ **The old "90-120 = +27u proven core" claim is FALSE on current data — don't trust it.**
