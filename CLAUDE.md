...
- **Live market** (`_live_market_prob`): the current **Polymarket implied probability** for the side (`pmm_markets.lookup` → `best_line_for` → `quote.mid`) — the market's live odds your bet hits.
- **Model estimate** (`_live_win_estimate`, fallback when PMM has no live price): a lightweight MLB live win-expectancy — `P(win) = Φ(edge / σ)`, `σ ∝ √(innings left)`, so σ→0 at the final out and a late lead reads ~100% (this is what catches a spread that already covered while ESPN still says "End 9th" and Polymarket has closed the market). Per market: ML on run lead, SPR on cover margin, TOT on projected final (`current + innings_left·0.95`). MLB only for now; other sports → grey. Decided > market > model > grey, surfaced as `prob_src` (`decided`/`market`/`model`/null). The gauge renders **model** estimates softer + a `~` prefix so a guess reads differently from a real market price.

PMM lookups are cached per `market_id` within the request; usually only a handful of live bets at once. **PMM in-play coverage is still unverified** — but the model fallback means the gauge fills sensibly even when Poly has no live line, so it never just greys out on an MLB game.
