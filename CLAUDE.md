...
**OUR-NUMBER-ONLY — there's no NRFI line in our odds feed.** The card shows the model fair %/American + a **lean vs the ~52.5% baseline** (`NRFI_LEAN_PP` = 4pp threshold); the user shops a book/Polymarket price beating the fair. A logged pick's `entry_price` = the model fair (the user can edit to their actual fill). `entry_line` is NULL.
