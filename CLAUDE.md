...
>
> **Pick Bot has TWO tiers within `bot_access` (revised late May 2026):** `bot_access` is now a **VIEW-ONLY** tier — those users (the friends following along) see the games list, the picks, the **units**, the colors, and the dossier read, but they **cannot log, cannot see the log** (pending/settled/stats/P&L), and **never see the green "logged" state**. **Logging is admin-only** — it exists for the admin's (Rob's) regression testing of the bot, so only admin writes/reads `bot_picks`. Client split: `_canBet` (VIEW = admin OR `bot_access`) gates page access + seeing picks; `_canLog` (admin only) gates logging + the log surfaces + green/upgrade states. Server backs it: dossier/games/sport-counts stay `@bot_required` (the read, zero log data); the stats payload + all pick-mutation routes are `@admin_required`.
...
32. **Pick Bot is two tiers: VIEW (`_canBet` = admin OR `bot_access`) and LOG (`_canLog` = admin only).** The bot's READ — `/api/handicapper/dossier`, `/games`, `/sport-counts` — carries NO logged-pick data (suggested pick / fair / splits / injuries only), so it's `@bot_required` and the view-only tier sees it (picks + units + colors + Details). The LOG — the stats payload `/api/handicapper` (pending/settled/P&L) AND every pick-mutation route (POST/DELETE/settle) — is `@admin_required` (logging is the admin's regression-testing tool, Rob only). Client: `_canBet` gates page access + the line-2 chips + the dossier read; `_canLog` gates `loadData` (stats/log), the green/upgrade/conflict states (`_pickState` returns `'fresh'` when `!_canLog`), one-tap logging, and every Log button (suggestion card, market grid, NRFI). `_applyViewerMode()` (runs when `!_canLog`) hides `#overallStats`/`#confStrip`/`#pendingSection`/`#settledSection`/betslip/refresh/status + the legend's `.legend-log` keys, and shows a VIEW-ONLY banner. **If a `bot_access` user ever sees logged data or a green chip, the bug is client-side gating** — but the server `@admin_required` on the stats + mutation routes is the real boundary (a crafted request 403s). Don't add logged-pick fields to the dossier endpoint — keep it strictly the live read (that's what lets the view tier see it safely).
