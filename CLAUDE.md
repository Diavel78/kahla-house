...
| `GET /api/handicapper/games?sport=X` | **Bot** | Lightweight list of active games for one sport — powers the click-to-pick UI on `/handicapper`. Window: events starting in next 48h (and within last 90 min so a late-asked game still appears). Returns `{market_id, event_name, event_start, away, home}` per game, sorted by event_start. The dossier (with odds / splits / injuries) is fetched on click via `/api/handicapper/dossier`. |
| `GET /api/handicapper/sport-counts` | **Bot** | One-shot count of upcoming games per sport — same pre-game window as `/api/handicapper/games` (last 90 min through next 48h). Returns `{counts: {MLB: 15, NBA: 0, ...}}`. Powers the dynamic sport-tab ordering on `/handicapper` (most-games-first; off-season sports drop to the right). |
...
- `kahla-scanner/supabase/bot_picks_migrations/` — manual SQL migrations for `bot_picks` schema changes. Run via Supabase SQL editor when the DDL changes (constraint additions, column renames, etc.). Numbered `00N_description.sql`. Idempotent. Current: `001_units_whale_tier.sql` (added 10u + whale confidence), `002_to_win_pnl.sql` (recompute pnl_units to to-WIN model), `003_resolver_heartbeat.sql` (resolver_runs heartbeat table — read by `/api/handicapper`).
...
**Multi-pick per game.** The dossier's `suggestions` field is a list. The bot may recommend up to 2 picks per game: one of {ML, SPR} plus an optional TOT. ML and SPR are mutually excluded — they're correlated bets on the same direction. Server-side exclusion picks the **better-priced** of the two (less-negative `fair_american` = bigger payout = the right version to take). Examples: ML +102 vs SPR -167 → ML wins; ML -169 vs SPR +120 → SPR wins. Falls back to combined_score if prices missing. The legacy `suggestion` field is kept as an alias for `suggestions[0]` for backward-compat.
...
**Multi-source public splits in the dossier.** `handicapper_web.py:_fetch_splits` blends three public sources per game, falling through gracefully when fewer match:
- **Action Network** (`api.actionnetwork.com/web/v2/scoreboard/{league}`) — the only source with **money %** (the sharp fingerprint). Bets % too.
- **Covers** (`covers.com/sport/{league}/picks-consensus`) — consensus pick % (≈ bets %). HTML scrape, best-effort parser.
- **VegasInsider** (`vegasinsider.com/{league}/matchups/`) — consensus picks. HTML scrape, best-effort parser.

Per-sport cached 30 min per source in module-level `_SPLITS_CACHE`. Match by two-way team-name substring containment (`Mariners` ↔ `Seattle Mariners`). `_blend_ml()` averages available bets % across whichever sources matched; money % stays Action-only (no other source publishes it). Returns `{away_bets, home_bets, away_money, home_money, sharp_diff, sources: [...]}`. Frontend uses `sources` to label the splits row (`blended (action + covers + vegasinsider)` vs single-source) and `_buildReasons` falls back to "Public X% on Y — money% unavailable" when only bets % was present. PIN line movement is still primary — public data is secondary, weighted 30% in `combined_score` only when the sharp side has movement (70% PIN sharp / 30% splits).

**Splits parsers are best-effort.** The Covers and VegasInsider parsers were written without browser verification of their HTML structure — selectors are educated guesses and will need 1-2 rounds of post-deploy tuning when sites redesign. Each fetcher returns rich `debug` info (URL, status, html_len, candidate_count, sample_blocks). When the splits row on `/handicapper` shows "blended (action)" instead of "blended (action + covers + vegasinsider)", that's the signal to hit `/debug-splits` (existing endpoint, Action-only) and add the equivalent for the new sources, then iterate the regex selectors. Action's path is verified and stable; Covers + VI are augmentation.

**Per-market grid is pick-scoped.** The Moneyline / Spread / Total breakdown panels under the suggestion card only render for markets the bot actually picked. ML-only pick → only ML grid shows. ML + TOT → those two. Cuts visual noise — when the bot has no read on SPR, you don't need to see the SPR numbers.
...
**Live odds on click.** The Pick button on the games list passes `live=true` to `/api/handicapper/dossier`, which hits The Odds API live (6 credits) so the dossier reflects the moment, not the last 30-min cron tick. Per-sport response cache (60s TTL, module-level dict in `handicapper_web.py`) so rapid clicks across multiple games in the same sport reuse one API call. Cold-start resets the cache. Search bar stays cached (no `live=true`) to avoid burning credits on typo retries. Cost ceiling at the worst-case usage of ~50 picks/day = ~9K credits/month, ~9% of the 100K tier. UI shows a green **LIVE ODDS** badge when fresh fetch succeeded, orange **CACHED · {reason}** when it fell back.
...
**Resolution / grading.** `bot_picks_resolver` runs every 30 min as the last step of `scanner-poll.yml` (`continue-on-error: true` on the workflow step). Was originally `RESOLVE_LAG_HOURS=4`; dropped to **0** — ESPN's `state='post'` is the authoritative game-over signal, so the resolver checks every pick whose `event_start` has passed and skips in-progress games (counted as `not_final`, retried next cron). Typical MLB game graded within ~30 min of final out. UFC ML auto-grades via the ESPN MMA endpoint (`mma/ufc`) using competitor `winner` boolean. UFC SPR/TOT method-of-victory bets stay pending — use the per-row Won/Lost/Push manual settle button on the page. **Visibility**: `resolver_runs` heartbeat table records every invocation (success or crash with full traceback). `/api/handicapper` returns the latest row; `/handicapper` header shows `graded Nm ago · NW/NL` (green) or `STALE` / `CRASHED` (red) so you can tell at a glance if grading is alive.
