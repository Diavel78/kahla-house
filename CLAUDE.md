...
| `/games` | `games.html` | admin + viewer (any approved) | **Games** — card-game scoring sheets. A `<select>` picks the game (only "Clubs · Diamonds · Hearts · Spades" so far); the sheet renders below. Fully client-side + offline: state persists to `localStorage` (`kh_games_cdhs_v1`), no backend/API/Firestore writes — the route just renders the static template (auth-gated client-side via `/api/me`, same pattern as `/odds`). Mobile-first (sticky hand column, horizontal scroll for many players). To add a game later: add a `<option>` + a HANDS/scoring block in `games.html`. **CDHS rules:** 17 hands — deal 7,6,5,4,3,2,1, then three 7-card middle hands (No Trump, Blind Diamonds = bid before seeing hand, Negative Spades = spades trump, every trick −10), then 1,2,3,4,5,6,7 back up. Trump cycles ♣♦♥♠ continuously across the numbered hands (middle hands don't advance it, so descending ends on ♥ and ascending picks up on ♠). Scoring: make bid exactly → bid×11; a made 0 bid → 10; miss → score = tricks taken. Each player may bid 0 at most 3× (soft counter). Total bids on a hand can't equal cards dealt (screw-the-dealer; shown as an indicator). Variable players (add/remove/rename), running totals + leader crown, New game clears scores but keeps players. |
...
- `templates/games.html` — **Games page**: self-contained card-game scoring sheets. No server logic beyond the `/games` render route in `app.py`; all state + scoring is client-side JS with `localStorage` persistence. First game = Clubs · Diamonds · Hearts · Spades (see `/games` row in Pages & Routes for the rules). Add new games by extending the `<select>` + a per-game HANDS/scoring block.
