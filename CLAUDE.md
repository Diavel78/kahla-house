...
38. **Vercel Fluid Active CPU is METERED — don't add per-minute server work casually.** June 2026 the per-minute `fill_ping` curls (paperlog ~8s of build_dossier + pm-snapshot ~7.5s of PMM lookups, every tick, all day) blew the Hobby tier's 4 CPU-hours/month in ~a week → "projects will be paused" email → upgraded the team to Pro. The fix that stays regardless of plan: **cadence gates live in `scanner-poll.yml`'s `fill_ping` job** (minute-modulo on the runner clock, BEFORE the curl, so the Vercel invocation never happens) — paperlog every 5 min (dedup is on bet change, 5-min sampling loses nothing), pm-snapshot every 2 min (free feeds lead PINN by ~4-7 min, so half the CPU keeps most of the trigger's latency edge), prime-alert every 5 min (30-min-wide batch window). **Only `/api/polymarket/check-fills` keeps true 1-min cadence** — fill alerts are the one latency-sensitive ping. Don't move these gates server-side (the invocation itself costs CPU) and don't quietly restore 1-min cadence "for freshness" — that's the burn. Pro overage is billed on-demand (~$0.13/Active-CPU-hour); a Spend Management cap in the Vercel dashboard is the backstop against a runaway loop.
