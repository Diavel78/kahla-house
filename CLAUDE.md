...
**Out-of-band SPR is filtered — SYMMETRIC ±150 (INVARIANT, do not half-code).** A SPR candidate is dropped before pick selection whenever its `fair_american` is outside `[-150, +150]` — too chalky (`<= SPR_CHALK_FAIR_CAP` -150, a worse-EV leveraged ML) OR too longshot (`>= SPR_LONGSHOT_FAIR_CAP` +150, a leveraged-dog runline with no edge). A spread only shows in the near-coinflip band where the leverage has value. **LANDMINE (June 2026):** only the chalk half (-150) was ever coded, so longshot runlines (e.g. `Reds -1.5 @ +192 fair / +292 entry`) leaked through as forced leans for months — the "+292 should never show" bug. Both halves now live in the `candidates` filter in `_suggest_picks`; keep them together. **Scoped to SPR only** — NOT ML (a +155 ML dog is a legit bet the user takes; a blanket +150 cap would block their own bets) and not TOT (always ~±120). ML at heavy chalk is also not filtered (no correlated alternative; the heavy ML IS the cleanest expression).
