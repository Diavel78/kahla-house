...
| `/api/handicapper/prime-alert` | **cron-only** | shared-secret `?key=PRIME_CRON_SECRET` OR `FILLS_CRON_SECRET` (NOT Firebase) — pinged every ~1 min by `scanner-poll.yml`'s `fill_ping` job. Batched Telegram (Filled Bot) when games enter the **180min (3h)** prime window; deduped via `prime_alerts`. See the prime-window note in the Pick Bot section. |
| `/api/pm-snapshot` | **cron-only** | shared-secret `?key=PM_SNAPSHOT_SECRET` OR `FILLS_CRON_SECRET` — pinged ~1/min by `fill_ping`. Logs free PMM+Kalshi cents per side for upcoming MLB games → `pm_snapshots` (the cross-confirm detector's memory + validation data). See "Prediction-market cross-confirm" section. |
| `/api/handicapper/paperlog` | **cron-only** | shared-secret `?key=PAPERLOG_SECRET` OR `FILLS_CRON_SECRET` — pinged ~1/min by `fill_ping`. Auto-logs every gate-cleared bot suggestion (5h→1min window) → `pickbot_paperlog`, deduped on bet-change. Budgeted ~8s. See "Pick Bot paperlog" section. |
...
**Make-vs-take execution engine (June 2026) — the directive chip + best-execution routing.** The maker-only default isn't always right: as tip nears, or when the book is heavily bid-stacked, a resting maker MISSES the bet — and sometimes a US-legal sportsbook simply has a better number than Polymarket's fee-inclusive take. So each gated suggestion gets a **live order-book verdict** telling the user exactly how to enter: `MAKE Polymarket 46¢` (rest a limit at the bid, fee-free, the cheapest+patient play) or `TAKE DraftKings +113` / `TAKE Polymarket 47¢` (cross now — routed to the single cheapest venue). **Backend `GET /api/handicapper/make-take`** (`@bot_required`; params `slug`,`market_id`,`side`,`units`,`starts_in_min`) reads the Polymarket book via `_pmm_book` → `_book_signal`:
...
> - **Best-execution routing (ML + spread + total).** On a TAKE, `_best_book_for(sb, market_type, side, line)` finds the cheapest **US-legal** book (`_ROUTE_BOOKS = DK, FD, MGM, ESPN, BET365, FAN` — the user's actual pool; NO Pinnacle/BetOnline, they're offshore) from latest `book_snapshots`, and routes there only if its implied beats Polymarket's **fee-inclusive** take (`take_eff` cents ≈ implied %, directly comparable to the book's raw american implied — a book carries no separate taker fee, so its american IS its all-in). Polymarket can still win. **Spread/total are matched at the SAME line** (the endpoint passes `market_type` + `line`; a book on a different number is a different bet, skipped — fails safe to PMM if no book matches the line). **LANDMINE (fixed June 2026):** `_best_book_for` (and the old `_best_book_ml`) compared the 0-1 implied *fraction* against the stored 0-100 *percent* when picking the cheapest book, so it always kept the LAST book scanned, not the best — the routing never actually found the cheapest price. Now compares `imp_pct` on the same scale. The endpoint's PMM-vs-book compare (`bb["implied"] < take_route["implied"]`) was always on the right (percent) scale.
>
> **Frontend (`handicapper.html`) — the directive rides the GAME CARD, the read lives on Details.** Per the user's mental model: the **games-list line-2 chip is "do this, this way"** and **Details is the explanation** — so the make/take pill's PRIMARY home is the game card, NOT the dossier. `_prefetchVerdicts` (the same background pass that builds the line-2 pick chips for in-window games) calls `_fetchMakeTakeVerdict` per gate-cleared pick that has a `pmm_slug`, attaches `_mt`/`_mtChip` (`_mtShort` → `MAKE Polymarket 46¢` / `TAKE DraftKings +113`), and `_renderPickLines` paints a quiet `.mt-pill` (green `.make` / amber `.take`) riding next to the bet chip inside a `.pick-chip-row` (hidden once logged / on a conflict — the directive is moot then). It refreshes with the 5-min verdict cache (TTL), NOT the 60s kickoff tick. **`_fetchMakeTakeVerdict` also populates `_makeTakeCache`, so a one-tap log from the chip enters at the recommended venue/price** (`_applyMakeTakeToBody` reads that cache). The **dossier** still renders its own `.mt-chip` + a verbose `.mt-detail` read (book touch / spread / fee / imbalance / the engine's one-line why) via `_loadAllMakeTake` at the end of `renderDossier` (re-runs on the 30s dossier loop) — that's where the full "why" lives. `_makeTakeCache` is keyed `mid:mt:side`. **History note: this was first built ONLY on the dossier (wrong surface — buried the directive behind Details); moved onto the game card per the original design.**
> - **Logging follows the rec** (`_applyMakeTakeToBody`, called in `submitLog` + `oneTapLog`): MAKE keeps `entry_book='PMM'` + `entry_price`=bid; **TAKE to a sportsbook sets `entry_book=DK/FD/...` + `entry_price`=that book's American; TAKE on PMM logs the fee-inclusive `take_eff` cents converted to American** (`_centsToAmer`) so CLV/to-WIN reflect the real fee cost. **No schema change** — the execution rec (`{rec, venue, kind, implied, entry_price, entry_book}`) is stashed in `signal_blob.execution` (jsonb). The 2-week make-vs-take review queries `signal_blob->'execution'->>'rec'`: did takes beat makes after fees, how often did makers miss. Chip + routing are gated to `gates_cleared` suggestions with a `pmm_slug`; viewer-safe (`_loadAllMakeTake` early-returns on `!_canBet`).
