...
> - **Best-execution routing (moneyline only for now).** On a TAKE, `_best_book_ml` finds the cheapest **US-legal** book (`_ROUTE_BOOKS = DK, FD, MGM, ESPN, BET365, FAN` — the user's actual pool; NO Pinnacle/BetOnline, they're offshore) from latest `book_snapshots`, and routes there only if its implied beats Polymarket's fee-inclusive take. Polymarket can still win. Spread/total takes always route to PMM (cross-book line routing is harder, not built).
>
