...
>
> **Frontend (`handicapper.html`):** the suggestion card renders a `.mt-wrap` placeholder per gated suggestion (carries `data-slug/mid/side/units/sim/mt`); `_loadAllMakeTake` (called at the end of `renderDossier`, so it **re-runs on the 30s dossier refresh loop** as the book moves) fetches the verdict and paints a quiet directive pill (`.mt-chip.make` green / `.mt-chip.take` amber) + a verbose `.mt-detail` read (book touch / spread / fee / imbalance / the engine's one-line why). **Per the user: the chip is the directive ("do this, this way"), the explanation lives in `.mt-detail` on the dossier — the dossier IS the details page.** The latest verdict is cached in `_makeTakeCache` keyed `mid:mt:side`.
