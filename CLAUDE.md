...
> - **Late-window sizing cap (`_timing_window` + `LATE_WINDOW_MIN`=60)** — picks made 1.5-2h pre-tip carried the entire edge (38 picks, 68.4%, +27u) while last-hour picks were mush (51 picks, 52.9%, +3.7u; totals negative). A pick made `< 60min` before first pitch is **capped at 1u** (never sizes up via Kelly — caps DOWN only) and tagged `late`; the **90-120min** window (the data's gold bucket, AND the upper bound is the 2h chip pick window — we don't pick past it) is tagged `prime` (green badge on the card), 60-90 is `normal`, >120 `early`. Picks carry `timing_window`/`late_capped`/`conflict_reason` into `signal_blob` so the next review can bucket by them. Both need `sport`/`home`/`starts_in_min` threaded into `_suggest_picks` (done in `build_dossier`).
| Conf | Units | When |
|---|---|---|
