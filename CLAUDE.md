...
> - **Queue-ahead / fill-risk (`_QUEUE_AHEAD_RATIO=3.0`, `_QUEUE_AHEAD_MIN=1000`)** — a resting maker joins the BACK of the line at the best bid; to fill, sellers must clear everyone ahead of it. This reads the **LEVEL-1 touch only** (`l1_bid/l1_ask`), NOT the 2-level sum — because a seller wall parked ONE level up dilutes the 2-level `imb` and HIDES a lopsided touch (real case: PMM total had best-bid 165,868 vs best-ask 35,046 = 4.7× buried, but the 49¢ ask wall pulled the 2-level imb down to 1.25× → it wrongly said MAKE). When `touch_imb ≥ 3.0` AND `l1_bid ≥ 1000`, the maker is buried and likely never fills (price pressure is UP — "the line keeps moving away from me") ⇒ **TAKE**, even if the 2-level imbalance looks balanced. Returns `touch_imbalance`/`queue_ahead`/`ask_touch`; the dossier `.mt-detail` shows `touch 47¢: 165,868 ahead vs 35,046 offered (4.7×)`.
...
> - **Time-aware.** The decision tree is clock-first: `≤30m to tip → TAKE` (a maker won't fill in time) **unless** the book is ask-heavy (`imb ≤ 0.4`, sellers will hit your bid fast → still MAKE); `≤60m + bid-heavy (imb≥2)` → TAKE; `imb≥3` → TAKE; `5u edge + imb≥2` → TAKE; else MAKE. (Unknown kickoff → no clock pressure.) This is the answer to "a pick popped in the prime window but it's 30m out and still unlogged — take should almost always win."
>
