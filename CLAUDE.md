...
> - **THE TAKE RULE (`_TAKE_IMB = 1.3`, user-rebuilt June 2026)** — **30% more contracts BID than OFFERED ⇒ ALWAYS TAKE**, unconditional (no clock/edge/size qualifiers). More buyers than sellers at the top = price pressure UP = a resting maker joins the back of a queue sellers aren't coming to clear. Checked on **BOTH reads, either fires**: the 2-level top-of-book sum (`imb`) AND the L1 touch (`touch_imb` = `l1_bid/l1_ask`) — the touch check exists because a seller wall parked ONE level up dilutes the 2-level `imb` and HIDES a lopsided touch (real case: best-bid 165,868 vs best-ask 35,046 = 4.7× buried, but a 49¢ ask wall pulled the 2-level imb to 1.25× → old logic wrongly said MAKE). **History:** the original tree (`imb≥3` / `≤60m+imb≥2` / `5u+imb≥2` / `touch≥3.0 AND l1_bid≥1000`) was the conservative first cut — user judged it "completely wrong" (kept saying MAKE on books where the maker plainly wouldn't fill) and set the flat 1.3× rule. Returns `touch_imbalance`/`queue_ahead`/`ask_touch`; the dossier `.mt-detail` shows `touch 47¢: 165,868 ahead vs 35,046 offered (4.7×)`.
> - **Fee-aware — the published take line is a lie.** `_pmm_taker_fee_cents(p)= 5·p·(1−p)` cents (max ~1.25¢ at 50/50; makers pay $0). The reported take price is **fee-inclusive** (`take_eff = ask + fee`), and the make-vs-take cost is `spread + fee` all-in. This is what flipped real games (SF@CHC) from TAKE to MAKE.
