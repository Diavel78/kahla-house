...
@app.route("/scanner")
def scanner_page():
    """Kahla Scanner review surface — admin only (client-side gated)."""
    return render_template("scanner.html")


# ---------------------------------------------------------------------------
# Scanner API — reads Supabase; admin-gated
# ---------------------------------------------------------------------------

@app.route("/api/scanner/activity")
@admin_required
def api_scanner_activity():
    import scanner as scanner_mod
    if not scanner_mod.is_configured():
        return jsonify({"ok": False, "error": "Supabase not configured"}), 503
    try:
        return jsonify({"ok": True, "data": scanner_mod.activity()})
    except Exception as e:
...
def api_scanner_signals():
    ...
    try:
        ...
        return jsonify({"ok": True, "data": scanner_mod.recent_signals(limit=limit)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/scanner/matches")
@admin_required
def api_scanner_matches():
    import scanner as scanner_mod
    if not scanner_mod.is_configured():
        return jsonify({"ok": False, "error": "Supabase not configured"}), 503
    try:
        limit = min(int(request.args.get("limit", 40)), 200)
        return jsonify({"ok": True, "data": scanner_mod.recent_matches(limit=limit)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/scanner/unmatched")
@admin_required
def api_scanner_unmatched():
    import scanner as scanner_mod
    if not scanner_mod.is_configured():
        return jsonify({"ok": False, "error": "Supabase not configured"}), 503
    try:
        limit = min(int(request.args.get("limit", 40)), 200)
        return jsonify({"ok": True, "data": scanner_mod.unmatched(limit=limit)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/scanner/brier")
@admin_required
def api_scanner_brier():
    import scanner as scanner_mod
    if not scanner_mod.is_configured():
        return jsonify({"ok": False, "error": "Supabase not configured"}), 503
    try:
        ...
        return jsonify({"ok": True, "data": scanner_mod.brier(sport=sport, days=days)})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
