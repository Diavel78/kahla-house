...
_PAPER_BOTS = ("steam", "early", "late")


def _paper_stats_empty():
    return {"graded": 0, "won": 0, "lost": 0, "push": 0,
            "units": 0.0, "hit_rate": None, "roi": None}


def _paper_stats_add(s, row):
    s["graded"] += 1
    st = row.get("status")
    if st in ("won", "lost", "push"):
        s[st] += 1
    try:
        s["units"] += float(row.get("pnl_units") or 0)
    except (TypeError, ValueError):
        pass


def _paper_stats_finalize(s):
    decided = s["won"] + s["lost"]
    s["hit_rate"] = round(s["won"] / decided, 4) if decided > 0 else None
    s["roi"] = round(s["units"] / s["graded"], 4) if s["graded"] > 0 else None
...
def api_sharp_bot():
    """JSON for /sharp-bot. Returns:
      pending          — every paper_bets row with status='pending'
      settled          — rows graded in the last 30 days (won/lost/push)
      stats            — per-bot rollup over `settled` (settled_at last 30d)
      stats_all_time   — per-bot rollup over EVERY graded paper_bets row
      stats_7d         — per-bot rollup over picks with event_start in last 7d
      stats_by_market  — per-(bot, market_type) rollup, all-time
      heartbeat        — per-bot last_pick_at / last_settle_at / overdue_pending
                         (overdue = status='pending' AND event_start < now)
    Rollups: hit_rate excludes pushes from denominator, units sums pnl_units,
    roi = units / graded. Null when no rows."""
    sb = get_supabase()
    if sb is None:
        return jsonify({"ok": False, "error": "Supabase not configured"}), 503
    ...
    rollup_cols = "bot,market_type,status,pnl_units,picked_at,settled_at,event_start"
    try:
        ...
        settled = (sb.table("paper_bets").select(cols)
                   ...
                   .limit(500).execute().data) or []
        all_graded = (sb.table("paper_bets").select(rollup_cols)
                      .neq("status", "pending")
                      .limit(10000).execute().data) or []
        all_pending = (sb.table("paper_bets").select(rollup_cols)
                       .eq("status", "pending")
                       .limit(2000).execute().data) or []
    except Exception as e:
        return jsonify({"ok": False, "error": f"Supabase: {e}"}), 500

    # 30d (existing — back-compat)
    stats = {b: _paper_stats_empty() for b in _PAPER_BOTS}
    for r in settled:
        if r.get("bot") in stats:
            _paper_stats_add(stats[r["bot"]], r)
    for s in stats.values():
        _paper_stats_finalize(s)

    # All-time, 7d (event_start), per (bot, market_type)
    stats_all_time = {b: _paper_stats_empty() for b in _PAPER_BOTS}
    stats_7d       = {b: _paper_stats_empty() for b in _PAPER_BOTS}
    stats_by_market: dict = {b: {} for b in _PAPER_BOTS}
    heartbeat = {b: {"total": 0, "graded": 0, "pending": 0,
                     "last_pick_at": None, "last_settle_at": None,
                     "overdue_pending": 0}
                 for b in _PAPER_BOTS}

    for r in all_graded:
        bot = r.get("bot")
        if bot not in stats_all_time:
            continue
        _paper_stats_add(stats_all_time[bot], r)
        if (r.get("event_start") or "") >= cutoff_7d:
            _paper_stats_add(stats_7d[bot], r)
        mt = r.get("market_type") or "?"
        bm = stats_by_market[bot].setdefault(mt, _paper_stats_empty())
        _paper_stats_add(bm, r)
        hb = heartbeat[bot]
        hb["total"] += 1
        hb["graded"] += 1
        s_at = r.get("settled_at") or ""
        if s_at and (hb["last_settle_at"] is None or s_at > hb["last_settle_at"]):
            hb["last_settle_at"] = s_at
        p_at = r.get("picked_at") or ""
        if p_at and (hb["last_pick_at"] is None or p_at > hb["last_pick_at"]):
            hb["last_pick_at"] = p_at

    for r in all_pending:
        bot = r.get("bot")
        if bot not in heartbeat:
            continue
        hb = heartbeat[bot]
        hb["total"] += 1
        hb["pending"] += 1
        ...
        if es and es < now_iso:
            hb["overdue_pending"] += 1

    for d in (stats_all_time, stats_7d):
        for s in d.values():
            _paper_stats_finalize(s)
    for bot_map in stats_by_market.values():
        for s in bot_map.values():
            _paper_stats_finalize(s)
...
@app.route("/sharp-bot.json")
def sharp_bot_json_page():
    """Mobile-friendly JSON view of /api/sharp-bot stats. Admin-gated
    client-side; copy the JSON straight to clipboard. `?full=1` to include
    the bulky pending/settled arrays."""
    full = "1" if request.args.get("full") in ("1", "true", "yes") else "0"
    return ('''<!DOCTYPE html><html><head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sharp Bot — stats JSON</title>
    <script src="https://www.gstatic.com/firebasejs/10.12.0/firebase-app-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.12.0/firebase-auth-compat.js"></script>
    <script>firebase.initializeApp({apiKey:"AIzaSyDQbjlc7VIYmFjbhq119Cl1-JhuXwKq0fY",authDomain:"kahla-house.firebaseapp.com",projectId:"kahla-house"});</script>
    <style>
      body{background:#0b0e13;color:#e2e8f0;font-family:'JetBrains Mono',ui-monospace,monospace;padding:14px;margin:0;font-size:12px;line-height:1.45}
      h2{color:#f59e0b;margin:0 0 10px;font-size:14px}
      .row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px}
      button,a.btn{background:#1f2533;color:#e2e8f0;border:1px solid #2a3140;border-radius:6px;padding:8px 12px;font:inherit;cursor:pointer;text-decoration:none}
      button:active,a.btn:active{background:#2a3140}
      button.primary{background:#4a7cff;border-color:#4a7cff;color:#fff}
      pre{background:#13171e;border:1px solid #1e2530;border-radius:6px;padding:12px;white-space:pre-wrap;word-break:break-word;-webkit-user-select:text;user-select:text}
      .msg{color:#8890a8;margin-bottom:8px}
    </style>
    </head><body>
    <h2>Sharp Bot · stats JSON</h2>
    <div class="row">
      <button class="primary" id="copy">Copy</button>
      <a class="btn" href="/sharp-bot.json''' + ('' if full == '1' else '?full=1') + '''">''' + ('Stats only' if full == '1' else 'Full payload') + '''</a>
      <a class="btn" href="/sharp-bot">Back to page</a>
    </div>
    ...
    firebase.auth().onAuthStateChanged(async u => {
        if (!u) { msg.textContent = 'Not logged in. Open / first.'; return; }
        try {
            const t = await u.getIdToken();
            ...
            const view = FULL ? d : {
                ...
                stats_7d: d.stats_7d,
                stats_30d: d.stats,
                stats_by_market: d.stats_by_market
            };
            out.textContent = JSON.stringify(view, null, 2);
            msg.textContent = 'Loaded ' + (new Date()).toLocaleString();
        } catch (e) {
